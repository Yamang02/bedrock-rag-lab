...
| `40-api` | IAM Role 관리 범위를 `role/bedrock-rag-lab-kb-role` 단일 ARN에서 `role/bedrock-rag-lab-*`(프로젝트 명명 규칙 전체)로 확장 (`ProjectRoleManage`로 개명), `iam:PassRole`을 `lambda.amazonaws.com`에도 허용, `lambda:*` 관리 action(함수 ARN 한정), `apigateway:*`(리전 전체 `arn:aws:apigateway:ap-northeast-2::*` — 처음엔 `/apis*`로 시작했으나 `/tags/*` 태깅 호출이 막혀 넓혔다), `logs:*Read`(CloudWatch Logs 조회, 함수 로그 그룹 한정) | Lambda + API Gateway로 CLI 전용이던 RAG를 HTTP API로 노출. 이 프로젝트는 짧게 쓰고 버릴 실습용이고 CI/CD(GitHub Actions)도 범위에서 제외하기로 해서, Role마다 별도 statement를 추가하는 대신 명명 규칙 기준 와일드카드로 단순화했다. |
| `40-api` (생성 모델 전환) | `bedrock:GetInferenceProfile`, Nova Micro의 inference profile ARN + 기반 foundation model ARN(리전 wildcard)으로 `InvokeModel` 대상 교체. `aws-marketplace:ViewSubscriptions`/`Subscribe`는 추가했다가 다시 제거. | Claude 3 Haiku가 Anthropic use case 미제출 → AWS Marketplace 구독 필요 순으로 막혀서, Marketplace 구독 없는 Amazon 자체 모델(Nova Micro, cross-region inference profile)로 교체했다. `GetInferenceProfile`은 KB Service Role(`infra/iam.tf`)뿐 아니라 실제 caller인 **Lambda 자신의 실행 Role**(`infra/lambda.tf`)에도 필요했다 — 처음엔 Service Role에만 추가해서 한 번 더 막혔다. 자세한 경위는 `docs/40-api/README.md` 부록 참고. |
| `50-cost-cleanup` | `budgets:ViewBudget`, `budgets:ModifyBudget` (리전 없는 글로벌 서비스라 Resource `*`) | `aws_budgets_budget`을 Terraform으로 생성하기 위해 필요. |
