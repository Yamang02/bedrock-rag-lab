...
| `40-api` | IAM Role 관리 범위를 `role/bedrock-rag-lab-kb-role` 단일 ARN에서 `role/bedrock-rag-lab-*`(프로젝트 명명 규칙 전체)로 확장 (`ProjectRoleManage`로 개명), `iam:PassRole`을 `lambda.amazonaws.com`에도 허용, `lambda:*` 관리 action(함수 ARN 한정), `apigateway:*`(HTTP API 리소스 경로 한정) | Lambda + API Gateway로 CLI 전용이던 RAG를 HTTP API로 노출. 이 프로젝트는 짧게 쓰고 버릴 실습용이고 CI/CD(50단계)도 제외하기로 해서, Role마다 별도 statement를 추가하는 대신 명명 규칙 기준 와일드카드로 단순화했다. `apigateway:*`는 API Gateway v2의 세분화된 하위 리소스 ARN 패턴(라우트/통합/스테이지별로 다름)을 일일이 맞추는 대신 편의 우선으로 넓게 잡았다 — 나중에 좁히는 걸 학습 포인트로 남겨둔다. |
